 ...
 *
 */
public class MimicVPNAccessEventSource extends RichParallelSourceFunction<LenelAccessEvent> {
	...
	public MimicVPNAccessEventSource(long pause, double temperatureStd, double temperatureMean) {
		this.pause = pause;
	...
	public void run(SourceContext<LenelAccessEvent> sourceContext) throws Exception {
		while (running) {
			for(String lenelRecord :Files.readAllLines(Paths.get("D:\\sample_vpn.csv"), StandardCharsets.UTF_8)) {
