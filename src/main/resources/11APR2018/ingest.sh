...
function export_collections {
	...
	. ./lib/custom_export.sh	
        ...
        log_warn "EXPORT Collections into directory = $(pwd)/export/";
	ls $(pwd)/export/
...
function update_elastic_search_index {
  speak "$1"
  log_info "updating elastic search stuff - approx taking below 30sec tentitively";
  . ./lib/esindex.sh
}
...
function has_arango_started {
speak "$1"
while true; do
  curl -X GET --silent -sS localhost:8529 -m 2;
  [ $? == 7 ] && log_debug "wating till arango start well" || break; 
done
sleep 3;
}

function has_elastic_started {
speak "$1"
while true; do
  curl -X GET --silent -sS localhost:9200 -m 2;
  [ $? == 7 ] && log_debug "wating till elastic start well" || break; 
done
sleep 3;
}

function analyse_links {
	speak "$1"
	has_arango_started
	has_elastic_started
}
