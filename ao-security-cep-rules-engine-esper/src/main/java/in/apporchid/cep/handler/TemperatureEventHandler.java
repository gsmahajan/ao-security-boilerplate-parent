...
import com.apporchid.beans.TemperatureEvent;
...
public class TemperatureEventHandler implements InitializingBean{
	...
	@Autowired
	@Qualifier("temperatureCriticalEventSubscriber")
	private StatementSubscriber temperatureCriticalEventSubscriber;

	@Autowired
	@Qualifier("temperatureWarningEventSubscriber")
	private StatementSubscriber temperatureWarningEventSubscriber;

	@Autowired
	@Qualifier("temperatureMonitorEventSubscriber")
	private StatementSubscriber temperatureMonitorEventSubscriber;
	...
	private void createCriticalTemperatureCheckExpression() {
		...
		criticalEventStatement = epService.getEPAdministrator().createEPL(temperatureCriticalEventSubscriber.getStatement());
		criticalEventStatement.setSubscriber(temperatureCriticalEventSubscriber);
	}
	...
	private void createWarningTemperatureCheckExpression() {
		...
		warningEventStatement = epService.getEPAdministrator().createEPL(temperatureWarningEventSubscriber.getStatement());
		warningEventStatement.setSubscriber(temperatureWarningEventSubscriber);
	}
	...
	private void createTemperatureMonitorExpression() {
		...
		monitorEventStatement = epService.getEPAdministrator().createEPL(temperatureMonitorEventSubscriber.getStatement());
		monitorEventStatement.setSubscriber(temperatureMonitorEventSubscriber);
	}
