 ...
 */
@Component
public class VpnMonitorDisconnectEventSubscriber implements StatementSubscriber {
	...
	public String getStatement() {
		...
		return "select count(*) as count from VpnEvent.win:time_batch("+timeWindow+" sec) where accountStatusType = 'Stop'";
	}
	...
	public void update(Map<String, Integer> eventMap) {
		...
		sb.append("---------------------------------");
		sb.append("\n- [MONITOR] VPN disconnect - total in last "+timeWindow+" seconds => " + eventMap.get("count"));
		sb.append("\n---------------------------------");
		...
		StartDemo.fakeBuilder.addActivity(new ActivityBuilder(EventSourceType.CYBER_VPN).withEpochTimeStart(Instant.now())
				...
				.withCount("monitor.disconnect.vpn", eventMap.get("count")).build());
